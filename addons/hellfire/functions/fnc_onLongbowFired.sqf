...
private _potentialTargetPos = if (_closestObject isEqualTo objNull) then { _cursorPos } else { getPosASL _closestObject };
...
_seekerStateParams set[1, CBA_missionTime];
...
if (!isVehicleRadarOn vehicle _shooter) then {
    ...
    private _aproxETA = (2 * _distanceToTarget) / (_missileAccel * _missileAccelTime);
